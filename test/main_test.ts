...
const createTestPerson = () =>
	new Person({
		...
		children: new Map([
			[
				'Alice',
				Person.createMinimal({ name: 'Alice', dob: new Date('2010-01-01') }),
			],
			[
				'Bob',
				Person.createMinimal({ name: 'Bob', dob: new Date('2012-01-01') }),
			],
		]),
		accounts: [
...
Deno.test(function serializeObjectWithPrimitives() {
	...
	const json = jcs.serializeToJson({
		a: 1,
		b: '2',
		c: true,
	...
	assertStrictEquals(
		json,
		'{"a":1,"b":"2","c":true,"d":"2000-01-01T00:00:00.000Z"}',
	)
})
...
Deno.test(function deserializeClassWithClassHint() {
	...
	const jcs = new JsonClassSerializer()
	const json = jcs.serializeToJson(person)
	const deserialized = new JsonClassSerializer().deserializeFromJson(
		json,
		Person,
	)
...
Deno.test(function deserializeWithManualClassResolution() {
	...
	const deserializedWithoutResolution = new JsonClassSerializer({
		failIfRootClassNotFound: false,
	}).deserializeFromJson(json)
	assertSimilarInstances(deserializedWithoutResolution, minimalPersonData)

	const deserializedWithClassHint =
		new JsonClassSerializer().deserializeFromJson(json, Person)
	assertSimilarInstances(deserializedWithClassHint, minimalPerson)

	const deserializedWithNameResolution = new JsonClassSerializer({
		deserializationClassResolver: () => 'Person',
	}).deserializeFromJson(json)
	assertSimilarInstances(deserializedWithNameResolution, minimalPerson)

	const deserializedWithClassResolution = new JsonClassSerializer({
		deserializationClassResolver: () => Person,
	}).deserializeFromJson(json)
...
Deno.test(function deserializeNestedStructures() {
	...
	const person = new Person({
		addresses: {
			...
			work: [new Address({ city: 'San Francisco' })],
		},
	})
	const jcs = new JsonClassSerializer()
	const json = jcs.serializeToJson(person)
...
Deno.test(function mapSerializationStrategyKeyValueObjects() {
	...
	const jcs = new JsonClassSerializer({
		mapSerializationStrategy: 'arrayOfKeyValueObjects',
	})
	const json = jcs.serializeToJson(map)
	assertStrictEquals(
		json,
		'{"#type":"Settings","map":[{"key":"a","value":1},{"key":"b","value":2}]}',
	)
	...
	const deserializedWithEntriesStrategy = new JsonClassSerializer({
...
Deno.test(function classInstanceWithInheritance() {
	...
	assertStrictEquals(
		json,
		'{"#type":"Cat","name":"Tom","character":"lazy","fur":"fuzzy"}',
	)
...
Deno.test(function anyTypeProperties() {
	...
	assertStrictEquals(
		json,
		'{"#type":"Foo","obj":{"a":1,"b":"2","c":true},"array":[1,"2",true],"map":[[1,"a"],["b",2]],"set":[1,"2",true]}',
	)
...
Deno.test(function deserializeUsingAlternativeTypeRegistry() {
	...
	assertSimilarInstances(
		deserializedNormal,
		Person.createMinimal({ name: 'John' }),
	)
	...
	const jcsWithoutAnyClassesButAllowingFailedResolutions =
		new JsonClassSerializer({
			useGlobalClassRegistry: false,
			failIfRootClassNotFound: false,
			failIfTypeResolutionFails: false,
		})
	const deserializedWithoutAnyClassesButAllowingFailedResolutions =
		jcsWithoutAnyClassesButAllowingFailedResolutions.deserializeFromJson(json)
	assertSimilarInstances(
		deserializedWithoutAnyClassesButAllowingFailedResolutions,
		JSON.parse(json),
	)
	...
	const deserializedWithAdditional =
		jcsWithAdditionalClasses.deserializeFromJson(json)
	assertSimilarInstances(
		deserializedWithAdditional,
		new Person_deserializeUsingAlternativeTypeRegistry({ name: 'John' }),
	)
})
...
Deno.test(function deserializeUnannotatedNestedClasses() {
	...
	const foo = new Foo()
	const jcs = new JsonClassSerializer()
	const jcsProhibitPlainRoot = new JsonClassSerializer({
...
Deno.test(function serializeBinaryData() {
	...
	const deserializedArrayBuffer = jcs.deserializeFromJson(
		jsonArrayBuffer,
		ArrayBuffer,
	)
...
Deno.test(function serializeClassWithBinaryData() {
	...
	const deserializedFromObj = jcs.deserializeFromObject(
		serializedToObj,
		HasBinaryData,
	)
...
Deno.test(function deserializeSubclasses() {
	...
	const deserializedContainer = jcs.deserializeFromJson(
		containerJson,
		Container,
	)
...
Deno.test(function serializeCircularDependenciesWithoutCorrectOption() {
	...
	assertStrictEquals(
		jsonWithRefsToSameObj,
		'{"foo":{"bar":{"a":1},"baz":{"a":1}}}',
	)
	const deserializedWithRefsToSameObj = jcs.deserializeFromJson(
		jsonWithRefsToSameObj,
	)
	assertSimilarInstances(deserializedWithRefsToSameObj, objWithRefsToSameObj)
	assertNotStrictEquals(
		deserializedWithRefsToSameObj.foo.bar,
		deserializedWithRefsToSameObj.foo.baz,
	)
...
Deno.test(function serializeCircularDependencies() {
	...
	parent.children = [new Person({ parent })]
